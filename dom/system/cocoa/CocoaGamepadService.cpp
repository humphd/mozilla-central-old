...
class CocoaGamepadService : public GamepadService {
 private:
...
{
  return new CocoaGamepadService;
}

void
CocoaGamepadService::DeviceAdded(IOHIDDeviceRef device)
{
...
void
CocoaGamepadService::DeviceRemoved(IOHIDDeviceRef device)
{
  for (size_t i = 0; i < mGamepads.size(); i++) {
    if (mGamepads[i] == device) {
...
void
CocoaGamepadService::InputValueChanged(IOHIDValueRef value)
{
  ...
  for (size_t i = 0; i < mGamepads.size(); i++) {
    ...
    if (gamepad == device) {
      if (const Axis* axis = gamepad.lookupAxis(element)) {
        ...
        NewButtonEvent(i, button->id, pressed);
      }
      return;
    }
  }
...
void
CocoaGamepadService::DeviceAddedCallback(void* data, IOReturn result,
                                         void* sender, IOHIDDeviceRef device)
{
  CocoaGamepadService* service = (CocoaGamepadService*)data;
  service->DeviceAdded(device);
}

void
CocoaGamepadService::DeviceRemovedCallback(void* data, IOReturn result,
                                           void* sender, IOHIDDeviceRef device)
{
  CocoaGamepadService* service = (CocoaGamepadService*)data;
...
void
CocoaGamepadService::InputValueChangedCallback(void* data,
                                               IOReturn result,
                                               void* sender,
...
CocoaGamepadService::CocoaGamepadService() : mManager(NULL) {}

CocoaGamepadService::~CocoaGamepadService()
{
  if (mManager != NULL)
    CFRelease(mManager);
}

void CocoaGamepadService::Startup()
{
  if (mManager != NULL)
    return;
...
void CocoaGamepadService::Shutdown()
{
